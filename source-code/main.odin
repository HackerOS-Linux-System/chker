...
install_liquorix :: proc() {
    ...
    liquorix_url := "https://liquorix.net/install-liquorix.sh"
    if !run_command("curl", "-s", "-f", "-o", "/dev/null", liquorix_url) {
        fmt.printf("%s Błąd pobierania skryptu Liquorix!\n", log_prefix)
        os.exit(1)
    }
    liquorix_script := "/tmp/install-liquorix.sh"
    if !run_command("curl", "-s", liquorix_url, "-o", liquorix_script) {
        fmt.printf("%s Błąd pobierania skryptu Liquorix do pliku!\n", log_prefix)
        os.exit(1)
    }
...
install_deb_kernel :: proc() {
    ...
    config_file := strings.join([]string{os.get_env("HOME"), ".config", "hackeros", "kernel.hacker"}, "/")
...
create_config_file :: proc(kernel_type: string) {
    config_dir := strings.join([]string{os.get_env("HOME"), ".config", "hackeros"}, "/")
    os.make_directory(config_dir, 0o755)
    config_file := strings.join([]string{config_dir, "kernel.hacker"}, "/")
    data := fmt.tprintf("[%s]\n", kernel_type)
    os.write_entire_file(config_file, transmute([]byte)data)
    _ = posix.chmod(cstring(raw_data(config_file)), { .IRUSR, .IWUSR, .IRGRP, .IROTH })
}
...
sort_mappings_by_length :: proc(mappings: []string) -> ([]string, []string) {
    Pair :: struct { pat: string, tar: string, len: int }
    pairs: [dynamic]Pair
    defer delete(pairs)
    for m in mappings {
...
command_exists :: proc(cmd: string) -> bool {
    desc := os2.Process_Desc {
        command = []string{"command", "-v", cmd},
    }
    state, out, err_out, err := os2.process_exec(desc, context.temp_allocator)
    defer delete(out)
...
run_sudo_command :: proc(args: ..string) -> bool {
    sudo_args := make([dynamic]string, len(args) + 1)
    defer delete(sudo_args)
    sudo_args[0] = "sudo"
    copy(sudo_args[1:], args)
    desc := os2.Process_Desc {
        command = sudo_args[:],
    }
    state, out, err_out, err := os2.process_exec(desc, context.temp_allocator)
...
run_command_output :: proc(args: ..string) -> string {
    ...
    res := strings.trim_space(string(out))
    delete(out)
    return res
}
